...
class AdminFrame(tb.Frame):
    ...
    '''def show_holdings_window(self, event):
        ...
        # 可用资金
        tb.Label(overview_frame, text="可用资金:", bootstyle="light").grid(row=0, column=0, sticky="w", padx=10, pady=5)
        balance_var = tk.StringVar(value=f"{user.get('balance', 0):.2f}")
        tb.Label(overview_frame, textvariable=balance_var, bootstyle="info").grid(row=0, column=1, sticky="w", padx=10, pady=5)
        
        # 持仓市值
        tb.Label(overview_frame, text="持仓市值:", bootstyle="light").grid(row=0, column=2, sticky="w", padx=10, pady=5)
        holdings_value_var = tk.StringVar(value=f"{holdings_value:.2f}")
        tb.Label(overview_frame, textvariable=holdings_value_var, bootstyle="info").grid(row=0, column=3, sticky="w", padx=10, pady=5)
        
        # 总资产
        tb.Label(overview_frame, text="总资产:", bootstyle="light").grid(row=1, column=0, sticky="w", padx=10, pady=5)
        total_value = user.get('balance', 0) + holdings_value
        total_value_var = tk.StringVar(value=f"{total_value:.2f}")
        tb.Label(overview_frame, textvariable=total_value_var, bootstyle="info").grid(row=1, column=1, sticky="w", padx=10, pady=5)
